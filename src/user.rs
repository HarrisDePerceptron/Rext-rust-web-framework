...
impl User {
    fn new(email: &str, password: &str, name: Option<String>) -> Result<User> {
...
impl UserPersist for UserDao {
    ...
    async fn get_user(&self, id: &str) -> Result<User> {
        ...
        let mut user = res.deserialize_current()?;
        //let oiid = res.current().get_object_id("_id")?;
        //let id = oiid.to_string();
        //user.id = Some(id);
        Ok(user)
    }
